...
exports.createUser = function (req, res, next) {  
    var user = new Student(req.body); //get data from ejs page and attaches them to the model
...
exports.authenticate = function(req, res, next) {
	...
	console.log(req.body)
	const studentNumber = req.body.auth.username;
	const password  = req.body.auth.password;
	console.log(password)
	console.log(studentNumber)
	//find the user with given studentNumber using static method findOne
	Student.findOne({username: studentNumber}, (err, student) => {
			if (err) {
				return next(err);
			} else {
			console.log(">>>>>>"+student)
			...
			if(bcrypt.compareSync(password, student.password)) {
				...
				res.status(200).send({ screen: student.usertype, type:student.usertype });
					console.log("success!")
				//call the next middleware
				next()
			} else {
